...
void leader_end_user(void) {
    if (leader_sequence_one_key(KC_TAB)) { // Leader, f => language change
        host_consumer_send(AC_NEXT_KEYBOARD_LAYOUT_SELECT);
        host_consumer_send(0);
    } /*else if (leader_sequence_one_key(KC_E)) { // Leader, e => €
        //send_unicode_string("€");
        unicode_input_start();
        register_hex(0x20AC);
        unicode_input_finish();
    } */else if (leader_sequence_one_key(QK_LEAD)) { // Leader, Leader => Caps Lock
        tap_code(KC_CAPS);
    } else {
        SEND_STRING(SS_LALT("`") );
    }
